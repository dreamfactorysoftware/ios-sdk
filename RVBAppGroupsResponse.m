...
@implementation RVBAppGroupsResponse
        ...
        if([record_dict isKindOfClass:[NSArray class]]) {
            ...
            if([(NSArray*)record_dict count] > 0) {
                for (NSDictionary* dict in (NSArray*)record_dict) {
                    RVBAppGroupResponse* d = [[RVBAppGroupResponse alloc] initWithValues:dict];
                    [objs addObject:d];
                }
...
-(NSDictionary*) asDictionary {
    ...
    if(_record != nil){
        if([_record isKindOfClass:[NSArray class]]){
            ...
            for( RVBAppGroupResponse *record in (NSArray*)_record) {
