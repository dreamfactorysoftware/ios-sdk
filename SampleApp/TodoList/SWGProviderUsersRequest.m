...
@implementation SWGProviderUsersRequest
        ...
        if([record_dict isKindOfClass:[NSArray class]]) {
            ...
            if([(NSArray*)record_dict count] > 0) {
                for (NSDictionary* dict in (NSArray*)record_dict) {
                    SWGProviderUserRequest* d = [[SWGProviderUserRequest alloc] initWithValues:dict];
                    [objs addObject:d];
                }
...
-(NSDictionary*) asDictionary {
    ...
    if(_record != nil){
        if([_record isKindOfClass:[NSArray class]]){
            ...
            for( SWGProviderUserRequest *record in (NSArray*)_record) {
