...
@implementation RVBContainerResponse
        ...
        if([folder_dict isKindOfClass:[NSArray class]]) {
            ...
            if([(NSArray*)folder_dict count] > 0) {
                for (NSDictionary* dict in (NSArray*)folder_dict) {
                    RVBFolderResponse* d = [[RVBFolderResponse alloc] initWithValues:dict];
                    [objs addObject:d];
                }
        ...
        if([file_dict isKindOfClass:[NSArray class]]) {
            ...
            if([(NSArray*)file_dict count] > 0) {
                for (NSDictionary* dict in (NSArray*)file_dict) {
                    RVBFileResponse* d = [[RVBFileResponse alloc] initWithValues:dict];
                    [objs addObject:d];
                }
...
-(NSDictionary*) asDictionary {
    ...
    if(_folder != nil){
        if([_folder isKindOfClass:[NSArray class]]){
            ...
            for( RVBFolderResponse *folder in (NSArray*)_folder) {
    ...
    if(_file != nil){
        if([_file isKindOfClass:[NSArray class]]){
            NSMutableArray * array = [[NSMutableArray alloc] init];
            for( RVBFileResponse *file in (NSArray*)_file) {
